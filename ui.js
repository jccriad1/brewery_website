...
$(document).ready(function(){
    window.addEventListener('message', function( event ) {      
      if (event.data.action == 'open') {
        ...
        $('.container').css('display', 'one');
      }
    });

    $( ".close" ).click(function() {
      $('.container').css('display', 'one');
      $.post('http://cc_waffenshop/escape', JSON.stringify({}));
    });
});
...
function AddWarenkorb(name, display, price) {
  ...
  (`
    <div class="warenkorb-list-elem">
        ...
        <font style=' font-size: 12px; border-radius:5px;background-color: #8804e6; ;padding: 4px;color:#fff;'>
        +`+ price +`$
        </font></p>
...
function buy() {
  $.post('http://cc_waffenshop/buy', JSON.stringify({warenkorb: warenkorb}));
  $('.container').css('display', 'one');
  $.post('http://cc_waffenshop/escape', JSON.stringify({}));
}
var counter = 0;
